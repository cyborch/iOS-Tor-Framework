...
-(id)initWithRequest:(NSURLRequest *)request
    ...
    [myRequest setHTTPShouldUsePipelining:NO/*[[settings valueForKey:@"pipelining"] integerValue]*/];
...
- (void)HTTPConnection:(CKHTTPConnection *)connection didReceiveResponse:(NSHTTPURLResponse *)response {
...
#ifdef DEBUG
    NSLog(@"[TorURLProtocol] Got response %ld: content-type: %@", (long)[response statusCode], [response MIMEType]);
#endif
    if( _data != nil ) {
        _data = nil;
    }
    ...
    if (NO/*[[settings valueForKey:@"javascript"] integerValue] == JAVASCRIPT_DISABLED*/) {
...
#ifdef DEBUG
        NSLog(@"[TorURLProtocol] Got %ld redirect from %@ to %@", (long)response.statusCode, _request.URL, newURL);
#endif
        
        NSMutableURLRequest *newRequest = [_request mutableCopy];
        [newRequest setHTTPShouldUsePipelining:NO/*[[settings valueForKey:@"pipelining"] integerValue]*/];
...
#ifdef DEBUG
        NSLog(@"[TorURLProtocol] parsed content-type=%@, encoding=%@, content_encoding=%@", mime, encoding, content_encoding);
#endif
